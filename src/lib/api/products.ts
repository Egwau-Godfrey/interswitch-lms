...
export const productsApi = {
  ...
  list: async (params?: ListParams & { is_active?: boolean }): Promise<PaginatedResponse<LoanProduct>> => {
    return apiClient.get<PaginatedResponse<LoanProduct>>('/products', params);
  },
  ...
  listActive: async (): Promise<LoanProduct[]> => {
    const response = await apiClient.get<PaginatedResponse<LoanProduct>>('/products', { is_active: true, page_size: 100 });
  ...
  create: async (data: LoanProductCreate): Promise<LoanProduct> => {
    return apiClient.post<LoanProduct>('/products', data);
  },
  ...
  get: async (productId: string): Promise<LoanProduct> => {
    return apiClient.get<LoanProduct>(`/products/${productId}`);
  },
  ...
  update: async (productId: string, data: LoanProductUpdate): Promise<LoanProduct> => {
    return apiClient.put<LoanProduct>(`/products/${productId}`, data);
  },
